...
func (t *TokenRepo) GetTokensByUserID(userID int) (*models.Token, error) {
	...
	err := t.DB.QueryRow(
		"SELECT id, user_id, access_hash, refresh_hash, created_at, updated_at FROM tokens WHERE user_id = ?",
		userID).Scan(&tokenModel.ID, &tokenModel.UserID, &tokenModel.AccessTokenHash, &tokenModel.RefreshTokenHash,
...
func (t *TokenRepo) CreateToken(tokenModel *models.Token) (*models.Token, error) {
	if tokenModel == nil {
		return nil, errors.New("incorrect token model")
	}
	if t.TX != nil {
		prepare, err := t.TX.Prepare("INSERT INTO users_tokens( user_id, access_hash, refresh_hash) VALUES ($1,$2,$3) RETURNING id")
		if err != nil {
			return nil, err
		}
		err = prepare.QueryRow(tokenModel.UserID, tokenModel.AccessTokenHash, tokenModel.RefreshTokenHash).Scan(&tokenModel.ID)
		if err != nil {
			return nil, err
		}
		return tokenModel, nil
	}
	prepare, err := t.DB.Prepare("INSERT INTO users_tokens( user_id, access_hash, refresh_hash) VALUES ($1,$2,$3) RETURNING id")
	if err != nil {
		return nil, err
	}
	err = prepare.QueryRow(tokenModel.UserID, tokenModel.AccessTokenHash, tokenModel.RefreshTokenHash).Scan(&tokenModel.ID)
...
func (t *TokenRepo) UpdateTokens(tokenModel *models.Token) (*models.Token, error) {
	if tokenModel == nil {
		return nil, errors.New("incorrect data to update")
	}
